...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT email, password, role, full_name FROM users WHERE email = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $row['password'])) {
            ...
            $_SESSION['full_name'] = $row['full_name']; // Store the user's name in the session
